...
def TrainImages(urls):
    ...
    try:
        ...
        print(e)
        return
    recognizer.save(os.path.join(Path(__file__).parent,
                    "TrainingImageLabel/trainner.yml"))

    train = open(os.path.join(Path(__file__).parent,
                 "TrainingImageLabel/trainner.yml"), 'rb')
    return train
...
def TakeImages(img):
    ...
    faceCascade = cv2.CascadeClassifier(
        os.path.join(Path(__file__).parent, harcascadePath))
