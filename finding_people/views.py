...
def UserLogout(request):
    if request.method == 'POST':
        ...
        try:
            tableData = LoggedInDataSerializer(LoggedInData.objects.get(session=session)).data
            
            data = {
                'username':tableData.get('username'),
                'session':''
            }
            loginSerializer = LoggedInDataSerializer(LoggedInData.objects.get(session=session),data=data,partial=True)
...
def checkForSessionIdExist(session):
    try:
        userData = LoggedInDataSerializer(LoggedInData.objects.get(session=session)).data
        return False
    except:
        return True
...
def FaceRecognize(request):
    ...
    if request.method == 'POST':
        ...
        tableData = TrackingUserDataSerializer(TrackingUserData.objects.filter(username=username),many=True).data
        idd = -1
        index = 0
    ...
    if request.method == 'GET':
        ...
        for i in serializerAadhar:
            ...
            for img in faceImg:
                ...
                res = SaveIntoTrainigImageTable(convertedImg,serial,'aadhar')
                if res==False:
                    break
                i+=1
        ...
        for i in firSerializer:
            ...
            for img in faceImg:
                convertedImg = cv2_to_normal_image(img,serial)
                res = SaveIntoTrainigImageTable(convertedImg,serial,'fir')
                if res==False:
                    break
                i+=1
        ...
        if res==True:
            ...
            try:
                cascadeData = CascadeAndTrainerData.objects.get(name='trainner')
                requests.put('http://127.0.0.1:8000/adm/cascadeandtrainer',files={'file_name':trainer},data={'name':'trainner'})
            except:
                requests.post('http://127.0.0.1:8000/adm/cascadeandtrainer',files={'file_name':trainer},data={'name':'trainner'})
            res = {
                'status':'success',
...
def AadharDetail(request):
    if request.method == 'GET':
        ...
        try:
            serializerAadhar = AadharDataSerializer(AadharData.objects.get(aadhar_number=aadhar_num)).data
...
def SaveIntoTrainigImageTable(img,serial,from_db):
    ...
    res = requests.post('http://127.0.0.1:8000/adm/trainimages',files={'image':img},data={'serial':serial,'from_database':from_db})
    status = res.json()
...
def CascadeFileAndTrainer(request):
    ...
    if request.method == 'PUT':
        ...
        cascadeSerializer = CascadeAndTrainerDataSerializer(cascadeData,data=userData)
        cascadeData.delete()
        if cascadeSerializer.is_valid():
            cascadeSerializer.save()
            if name=='cascade':
